...
if(isset($_REQUEST['action']) && !empty($_REQUEST['action'])){
    if($_REQUEST['action'] == 'addToCart' && !empty($_REQUEST['id']) && !empty($_REQUEST['pageurl'])){
		...
		$value = $_SESSION["shippingcost"];
        $discount = $_SESSION['couponPrice'];
        $paymentoption =  $_GET['paymentType'];
        
        $insertOrder = $db->query("INSERT INTO orders (customer_id, total_price, sub_total_price, shipping_cost, discount_amount, payment_method, status, created) VALUES ('".$_SESSION['userid']."', '".$_SESSION['checkOutPrice']."','".$_SESSION['checkOutPricett']."', '".$value."','$discount','$paymentoption', '1', Now())");
