		...
		<form method="post" id="user_form" enctype="multipart/form-data">
			<div class="modal-content">
				...
				<div class="modal-body">
					...
					<b style="color:red">***</b><label> Enter Delivery Boy Full Name</label>
					<input type="text" name="delv_full_name" id="delv_full_name" class="form-control" />
					<br />	
					<b style="color:red">***</b><label> Enter Phone Number</label>
					<input type="text" name="delv_phone" id="delv_phone" class="form-control" />
					<br />	
...
$(document).ready(function(){
	...
	$(document).on('submit', '#user_form', function(event){
		...
		var delv_full_name = $('#delv_full_name').val();
		var delv_phone = $('#delv_phone').val();
		var remarks = $('#remarks').val();
		var active = $('#active').val();
		
		if(delv_full_name != '' && delv_phone !='' && active != '')
		{
			$.ajax({
				url:"insert.php",
				method:'POST',
	...
	$(document).on('click', '.update', function(){
		...
		$.ajax({
			...
			success:function(data)
			{
				...
				$('#delv_full_name').val(data.delv_full_name);
				$('#delv_phone').val(data.delv_phone);
				$('#remarks').val(data.remarks);
				$('#active').val(data.active);
				$('.modal-title').text("Edit Delivery Man Information");
				$('#user_id').val(user_id);
				$('#action').val("Edit");
				$('#operation').val("Edit");
			}
