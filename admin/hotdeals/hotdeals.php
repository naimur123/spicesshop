...
<html>
	...
	<body>
		...
		<div class="container-fluid">
			...
			<div class="row">
				<h2 class="h2" align="center"><b>Hot Deal Items Show</b></h2>
			</div>
			...
			<div class="row">
				...
				<div align="right">
					<button type="button" id="add_button" data-toggle="modal" data-target="#userModal" class="btn btn-info"><span class="glyphicon glyphicon-plus-sign"></span> Add Hot Deal Items</button>
				</div>
				<br/>
				<table id="user_data" class="table table-bordered table-striped">
					<thead>
						<tr>
							...
							<th>Price</th>
							<th>Hot Deal Item</th>
							<th>Hot Deal Percentage</th>
							<th>Hot Deal Price</th>
							<th>Flag</th>
							<th>Edit</th>
							<th>Delete</th>
						</tr>
		...
		<form method="post" id="user_form" enctype="multipart/form-data">
			<div class="modal-content">
				...
				<div class="modal-body">
					...
					<br />
					<b style="color:red">***</b><label> Select Hot Deal Item</label>
					<select class="form-control" name="offitem" id="offitem">
			      	<option value="">SELECT Hot Deal Item</option>
			      	<option value="1">Activate</option>
			      	<option value="0">Deactivate</option>
					</select>
					<br />
					<b style="color:red">***</b><label> Enter Hot Deal Price</label>
					<input type="text" name="offprice" id="offprice" class="form-control" />
					<br />
					<label> Enter Hot Deal Percentage</label>
					<input type="text" name="offpercent" id="offpercent" class="form-control" />
					<br />
...
$(document).ready(function(){
	$('#add_button').click(function(){
		$('#user_form')[0].reset();
		$('.modal-title').text("Add New Hot Deal Item");
		$('#action').val("Add");
		$('#operation').val("Add");
	...
	$(document).on('submit', '#user_form', function(event){
		...
		if(item_name != '' && category_name != '' && quantity!='' && price!='' && active!='' && offeritm!='' )
		{
	...
	$(document).on('click', '.update', function(){
		...
		$.ajax({
			...
			success:function(data)
			{
				...
				$('#offitem').val(data.offitem);
				$('#offprice').val(data.offprice);
				$('#offpercent').val(data.offpercent);

				$('.modal-title').text("Edit Hot Deal Item");
				$('#user_id').val(user_id);
