...
if(isset($_POST["user_id"]))
{
	...
	foreach($result as $row)
	{
		...
		$output["offitem"] = $row["offer_item"];
		$output["offpercent"] = $row["offer_percentage"];
		$output["offprice"] = $row["offer_price"];
		
		if($row["name"] != '')
		{
			$output['user_image'] = '<img src="../items/'.$row["name"].'" class="img-thumbnail" width="50" height="35" /><input type="hidden" name="hidden_user_image" value="'.$row["name"].'" />';
		}
		else
		{
