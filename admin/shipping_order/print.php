...
class myPDF extends FPDF{
    function header(){
        $this->image('11.png',10,6,80);
         $this->SetFont('Arial','B',20);
         $this->Cell(0,5,'SpicesShop.com',0,0,'R');
        // $this->Ln();
         $this->Ln();
...
function customerviewAddress($db){
        ...
        $stmt = $db->query('SELECT orders.created, orders.id, customer_registration.full_name, customer_registration.address,customer_registration.customer_area,customer_registration.phone,customer_registration.district FROM orders JOIN customer_registration ON orders.customer_id=customer_registration.id and orders.id = '.$rec_id);
        while($data = $stmt->fetch(PDO::FETCH_OBJ)){
			...
			$this->Ln();
			$this->Cell(22,5,'District: ',0,0,'L');
			$this->Cell(0,5,$data->district,0,0,'L');
			$this->Ln();
